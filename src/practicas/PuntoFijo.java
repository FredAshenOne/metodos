...
public class PuntoFijo extends JFrame implements ActionListener, MouseListener {
	...
	public PuntoFijo() {
		...
		JLabel lblHeader = new JLabel("Punto Fijo");
		...
		tbBiseccion.setModel(
				new DefaultTableModel(new Object[][] {}, new String[] { "Iteracion", "x", "f(x)", "Error Relativo" }));
	...
	public double puntoFijo(double valor) {
		return Math.sqrt(2 * valor + 3);
	}
			...
			if (isInt(txtHelp.getText())) {
				...
				if (isDouble(txtValue1.getText()) || isInt(txtValue1.getText())) {
					...
					for (int i = 0; i < iteraciones; i++) {
						valorActual = puntoFijo(valorAnterior);
						errorAbsoluto = calcularErrorAbs(valorAnterior, valorActual);

						model.addRow(new Object[] { i + 1, valorActual, puntoFijo(valorActual), errorAbsoluto });
