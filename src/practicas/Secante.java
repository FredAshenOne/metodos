...
public class Secante extends JFrame implements ActionListener, MouseListener {
	...
	public Secante() {
		...
		JLabel lblHeader = new JLabel("Secante");
		...
		tbBiseccion.setModel(new DefaultTableModel(new Object[][] {},
				new String[] { "Iteracion", "Limite A", "Limite B", "Funcion", "Error Relativo" }));
	...
	public double secanteVa(double a, double b) {
		return b - ((b - a) * secanteFuncion(b) / (secanteFuncion(b) - secanteFuncion(a)));
	}

	public double secanteFuncion(double x) {
				...
				if (isInt(txtValue1.getText()) && isInt(txtValue2.getText())) {
					...
					if (!isNotSqrt(getLimitA(), getLimitB())) {
						...
						for (int i = 0; i < iteraciones; i++) {
							valorActual = secanteVa(getLimitA(), getLimitB());
							errorAbsoluto = Math.abs((valorActual - valorAnterior) / valorActual) * 100;
							model.addRow(new Object[] { i + 1, getLimitA(), getLimitB(), secanteFuncion(valorActual),
									errorAbsoluto });
							valorAnterior = valorActual;
							if (secanteFuncion(getLimitA()) * secanteFuncion(valorActual) < 0) {
								setLimitA(getLimitB());
		...
		case 1:
			...
			if (isDouble(txtHelp.getText())) {
				if (isInt(txtValue1.getText()) && isInt(txtValue2.getText())) {
					orderLimits();
					if (!isNotSqrt(getLimitA(), getLimitB())) {
						double evMin = Double.parseDouble(txtHelp.getText());
						do {

							valorActual = secanteVa(getLimitA(), getLimitB());
							errorAbsoluto = Math.abs((valorActual - valorAnterior) / valorActual) * 100;
							model.addRow(new Object[] { j + 1, getLimitA(), getLimitB(), secanteFuncion(valorActual),
									errorAbsoluto });
							valorAnterior = valorActual;
							if (secanteFuncion(getLimitA()) * secanteFuncion(valorActual) < 0) {
								setLimitA(getLimitB());
							...
							j++;
							lblWarning.setText("");
						} while (evMin < secanteFuncion(valorActual));
						tbBiseccion.setModel(model);
					} else {
						lblWarning.setForeground(Color.RED);
						lblWarning.setText("El intervalo no tiene Raiz");
	...
	public boolean isNotSqrt(double limita, double limitn) {
		return secanteFuncion(limita) * secanteFuncion(limitn) > 0;
	}
